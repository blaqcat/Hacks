...
from time import gmtime, strftime   # import the time function
...
def portScan(event):
    ...
    # First, I need to check that the starting port id <= ending port value
    ...
    mainWin.StatusBar.SetStatusText().SetStatusText('Executing Port Scan .... Please Wait')
    ...
    # Extract data from the ip Range and host name name user selections
    # Build a Python List ofIP Addresses to Sweep
    ...
    for port in range(portStart.GetValue(), portEnd.GetValue()+1):
        ...
        try:
            # Reopen the IP Addresses to Window Status Bar
            ...
            # then display the sults received
            ...
            results.AppendText(baseIP+'\t'+str(port)+']t')
...
# End Progrma Exit Event Handler ==================================================
...
# I'm creating buttons, one for Scan and one for Exit
...
scanButton.Bind(wx.EVT_BUTTON, programExit)
...
# the last 8 bit define the host i.e 0-255

# Thus I setup 3 spin controls one for each of thw 4 network octets
...
ipLabel = wx.StaticText(panelAction, label="IP Addresses: ")

# Next, Iwant to provide the user with the ability to set the port range
...
actionBox.Add(displayAll, proportion=0, flag=wx.LEFT, border=5)
...
actionBox.Add(portEnd,proportion=0, flag=wx.LEFT | wx.CENTER)
...
# i'm adding a menu and status bar to the the main window

mainWin.CreateStatusBar()

# Finally, I use the SetSizer funvtion to automatically size the windows
